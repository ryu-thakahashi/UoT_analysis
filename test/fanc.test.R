...
glimpse(for.df);table(for.df$forgive1.1)

psych::alpha(for.df, check.keys = TRUE)

for.revCont.df = 6 - for.df %>% 
...
for.Cont.df = for.df %>% 
  dplyr::select(ends_with("1"),
                ends_with("4"),
                ends_with("5"))
for.df = cbind(for.revCont.df,for.Cont.df)
glimpse(for.df)

psych::alpha(for.df)

forRev.mean = rowMeans(for.df)
glimpse(forRev.mean)
glimpse(df$forgive)
